    ...
    float posY, std::function<void(GameObject*)> onClick, float delay, float scale, float posX)
{
    ...
    float btnX = (posX < 0)
        ? (screenW / 2.0f) - (btnW / 2.0f)  // centr� par d�faut
    ...
    float txtX = (posX < 0)
        ...
        : posX - (txtBounds.size.x / 2.0f) - txtBounds.position.x; // centr� sur posX
...
void Scenes::Start()
{
    ...
    // Callback apr�s changement de r�solution
...
OptionData Scenes::CreateOption()
{
    ...
    // Bouton + align� sur le centre du texte
